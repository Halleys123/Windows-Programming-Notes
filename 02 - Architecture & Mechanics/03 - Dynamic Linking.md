...
Internal working of DLL is much more complex including the things like handling stack, heap memory, local, global variables but that is not required for just using or creating a DLL and hence won't be discussed here. ([[Study Later ToDo#^bfceee|Although I tried understanding it but was not able to do so hence skip for now]])